...
>(({ className, ...props }, ref) => (
  <div ref={ref} className={cn("w-80 flex flex-col leftnav", className)} {...props} />
));
...
const MenuItem: React.FC<MenuItemProps> = (props) => {
  return (
    <Link
      className={cn(
        props.className,
        "items-center text-sm font-medium flex gap-2 p-2 py-1 rounded-md hover:bg-primary/40",
        props.isSelected && "bg-primary/40"
      )}
      href={props.href}
    >
      {props.children}
